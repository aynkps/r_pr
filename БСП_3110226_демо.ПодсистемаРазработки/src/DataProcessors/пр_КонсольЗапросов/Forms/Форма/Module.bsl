	...
	Для каждого СтрЗапросы Из Объект.Запросы Цикл
		...
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда
			...
			РезультирующаяСтрока	= НСтр("ru = 'Результат запроса (количество строк = %КолСтрок%, время выполнения = %ВремяВыполнения% с)';
											|en = 'Query result (number of strings = @@%КолСтрок%, execution time = %ВремяВыполнения% from)'");
...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	//TODO:  Доработка
	пр_МВТ = Объект.пр_МВТ.НайтиСтроки(Фильтр);
	...
	Результат = ОбъектОбработки().ВыполнитьЗапрос(Текст, МассивПараметров, ТабличныйДокументРезультата, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса, МеткаЗапроса, пр_МВТ);
	...
	РезультирующаяСтрока = НСтр("ru = 'Результат запроса (количество строк = %КоличествоСтрок%, время выполнения = %ВремяВыполнения% с)';
								|en = 'Query result (number of strings = %КоличествоСтрок%, execution time = %ВремяВыполнения% from)'");
...
Процедура СохранитьФайлЗапросаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		СообщитьПользователю(НСтр("ru = 'Без расширения для работы с 1С:Предприятием невозможно работать с файлами.';
									|en = 'To manage files, install the 1C:Enterprise extension.'"), "Объект");
	КонецЕсли;
...
Процедура ЧтениеФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		СообщитьПользователю(НСтр("ru = 'Без расширения для работы с 1С:Предприятием невозможно сохранять и загружать запросы в файл.';
									|en = 'Без расширения для работы с 1С:Предприятием невозможно сохранять и загружать запросы в файл.'"), "Объект");
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыИзXMLСервер(Знач СтрокаXML, Знач Контекст)
	...
	Если Контекст.ИзТекстаЗапроса Тогда
		...
		Фильтр = Новый Структура;
		Фильтр.Вставить("ИдентификаторЗапроса", ИдентификаторТекущегоЗапроса);
		пр_МассивМВТ = ЗначениеИзСтрокиВнутр(СтруктураПараметров.МВТ);
		Если пр_МассивМВТ <> Неопределено Тогда
			Для каждого ЭлМас Из пр_МассивМВТ Цикл
				Фильтр.Вставить("ПолноеИмя", ЭлМас.ПолноеИмя);
				Найденные = Объект.пр_МВТ.НайтиСтроки(Фильтр);
				Если Найденные.Количество() = 0 Тогда
					пр_МВТ = Объект.пр_МВТ.Добавить();
					пр_МВТ.ПолноеИмя = Фильтр.ПолноеИмя;
					//ПараметрЗапроса.Идентификатор = Новый УникальныйИдентификатор;
					пр_МВТ.ИдентификаторЗапроса = ИдентификаторТекущегоЗапроса;
				Иначе
					пр_МВТ = Найденные[0];
				КонецЕсли;
				
				пр_МВТ.Значение = ЗначениеВСтрокуСервер(ЭлМас.Данные);
				пр_МВТ.КолВо 	= ЭлМас.КолВо;
				//пр_МВТ.ЗначениеВФорме = ЭлМас.Данные;
...
&НаКлиенте
Процедура пр_ЗаполнитьИзТекстовогоФайла(Команда)
	пр_ПрочитатьИзФайла();  
КонецПроцедуры

&НаКлиенте
Функция пр_ПрочитатьИзФайла()
	
	Попытка
		
		Путь = ТекстЗапроса.ПолучитьТекст();
		ТекстовыйФайл = Новый ТекстовыйДокумент;
		ТекстовыйФайл.Прочитать(Путь);
		пр_ТекстЗапросаXML = ТекстовыйФайл.ПолучитьТекст();
		//пр_ЗапросИзСтроки(пр_ТекстЗапросаXML);

		
		ТекстЗапроса.УстановитьТекст(пр_ТекстЗапросаXML);
...
&НаСервере
Функция пр_ЗапросИзСтроки(пр_ТекстЗапросаXML)
	пр_Запрос = ЗначениеИзСтрокиВнутр(пр_ТекстЗапросаXML);
	
КонецФункции 

&НаКлиенте
Процедура пр_МВТВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	пр_МВТВыборНаСервере();
КонецПроцедуры

//TODO:  Доработать вывод МВТ
&НаСервере
Процедура пр_МВТВыборНаСервере()
