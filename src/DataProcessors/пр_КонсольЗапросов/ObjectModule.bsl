...
//
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, ТДРезультатаЗапроса, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса, МеткаЗапроса, пр_МассивМВТ = Неопределено) Экспорт
	...
	Если пр_МассивМВТ <> Неопределено Тогда
		Если пр_МассивМВТ.Количество() > 0 Тогда
			
			МВТ = Новый МенеджерВременныхТаблиц;
			
			//пр_Модуль = пр_Общий.ИсполняемыйМодуль("пр_Общий", пр_НастройкиПовтИсп.ТекущийПользователь()); 
			//пр_Модуль.ДополнитьМВТ(МВТ, пр_МассивМВТ);
			пр_Общий.ДополнитьМВТ(МВТ, пр_МассивМВТ);
